...
def home():
    ...
    # set of favorited artwork ids for the current user (for heart icon state)
    favorite_ids = set()
...
def update_artwork(artwork_id):
    ...
    # --- 7. Role management: only Admin or the artwork's owner may edit ---
...
def delete_artwork(artwork_id):
    ...
    # --- 7. Role management: only Admin or the artwork's owner may delete ---
    if not current_user.is_admin() and artwork.user_id != current_user.id:
...
def artwork_details(artwork_id):
    ...
    # --- 2. Comments & ratings (POST handling) ---
